...
if (!Array.prototype.max) {
  Array.prototype.max = function () {
    ...
    if (Number.isInteger(arguments[0])) {
      var n = arguments[0];
      var comparator = arguments[1] || function (a, b) { return a.compareTo(b) };
      return this.sort(comparator).slice(this.length - n, this.length).reverse();
    }

    // Array#max() [{ |a, b| block }]
    var comparator = arguments[0] || function (a, b) { return a.compareTo(b) };
    var maxValue = this[0];
    for (var i = 1; i < this.length; i++) {
      if (comparator(this[i], maxValue) > 0) {
        maxValue = this[i];
      }
    }
    return maxValue;
...
if (!Array.prototype.sortBy) {
  Array.prototype.sortBy = function (block) {
    return this.sort(function (a, b) {
      return block(a).compareTo(block(b));
    });
  };
}
